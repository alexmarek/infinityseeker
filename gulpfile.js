...
gulp.task( 'watch', function() {
    ...
    gulp.watch( [paths.dev + '/js/**/*.js', 'js/**/*.js', 'js/map.js', 'js/map-de.js', 'js/map-fr.js', '!js/child-theme.js', '!js/child-theme.min.js'],  ['scripts'] );
...
gulp.task("maps", function () {
    return gulp.src(paths.js + '/map.js')
        .pipe(rename("map.min.js"))
        .pipe(uglifyes(/* options */))
        .pipe(gulp.dest(paths.js));
});

gulp.task("maps-de", function () {
    return gulp.src(paths.js + '/map-de.js')
        .pipe(rename("map-de.min.js"))
        .pipe(uglifyes(/* options */))
        .pipe(gulp.dest(paths.js));
});

gulp.task("maps-fr", function () {
    return gulp.src(paths.js + '/map-fr.js')
        .pipe(rename("map-fr.min.js"))
        .pipe(uglifyes(/* options */))
        .pipe(gulp.dest(paths.js));
});
...
gulp.task( 'clean-vendor-assets', function() {
  return del( [paths.dev + '/js/bootstrap4/**', paths.dev + '/sass/bootstrap4/**', './fonts/*wesome*.{ttf,woff,woff2,eot,svg}', paths.dev + '/sass/fontawesome/**', paths.dev + '/sass/underscores/**', paths.dev + '/js/skip-link-focus-fix.js', paths.js + '/**/skip-link-focus-fix.js', ( paths.vendor !== ''?( paths.js + paths.vendor + '/**' ):'' )] );
});
